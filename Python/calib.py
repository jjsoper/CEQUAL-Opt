...
RMSE: BN Temp, BN Cond, CI Temp, CI Cond
*Remove CI Cond objective for calibration prior to 2016 due to probe errors
"""
...
class cequal(FloatProblem):
    ...
    def write_wsc(self, wsc_vals, regions, file_path):
        os_fold = os_sep()
        wsc= pd.read_csv("{}{}".format(file_path, os_fold + "wsc.csv"), header=None)
        sim_days = wsc.loc[4,0]
        branch = [len(section) for section in regions.values()]
        wsc.loc[2,1:] = np.concatenate([value for value in regions.values()])
        index = 0
    ...
    def write_full_outputs(self, vars, all_objs, trial_name, year):
        os_fold = self.os_sep()
        f = open(os.getcwd() + os_fold + 'results' + os_fold + year + os_fold + trial_name + '_all_runs.txt', 'a')
        for item in vars:
            f.write("%s," % item)
    ...
    # jMetalPy evaluate function: return objectives (RMSE) from objective function (CE-QUAL)
...
if __name__ == '__main__':
    ...
    os.chdir('..')
    year = '2017'
    trial_name = "2017_debug"

    run_optimization(max_eval = 8, num_nodes = 4, pop_size = 4, offspring = 4, trial_name = trial_name, year = year)
